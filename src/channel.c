    ...
    while (1) {
        ...
        switch (reply_type) {
            case SSH_MSG_CHANNEL_OPEN_CONFIRMATION:
                ...
                rc = ssh_buffer_unpack(session->in_buffer, "ddd", &recipient_channel,
                                       &channel->remote_window, &channel->remote_maxpacket);

                if (rc != SSH_OK) {
                    LOG_ERROR("can not unpack buffer");
                    return SSH_ERROR;
                }

                if (recipient_channel != channel->local_channel) {
                    LOG_ERROR("channel id mismatch");
                    return SSH_ERROR;
                }
            ...
            case SSH_MSG_CHANNEL_OPEN_FAILURE:
                ...
                switch (reason_code) {
                    case SSH_OPEN_ADMINISTRATIVELY_PROHIBITED:
                        LOG_ERROR("administratively prohibited");
                        break;
                    case SSH_OPEN_CONNECT_FAILED:
                        LOG_ERROR("connect failed");
                        break;
                    case SSH_OPEN_UNKNOWN_CHANNEL_TYPE:
                        LOG_ERROR("unknown channel type");
                        break;
                    case SSH_OPEN_RESOURCE_SHORTAGE:
                        LOG_ERROR("resource shortage");
                        break;
                    default:
                        LOG_ERROR("unknown reason code");
                        break;
                }

                LOG_ERROR("channel open failure description: %s", description);
            ...
            case SSH_MSG_GLOBAL_REQUEST:
                ...
                if (want) {
                    rc = ssh_buffer_pack(session->out_buffer, "bSb", SSH_MSG_REQUEST_FAILURE, req, 0);
                    if (rc != SSH_OK) {
                        LOG_ERROR("can not create buffer");
                        return SSH_ERROR;
                    }
            ...
            default:
                // LAB: insert your code here.
                LOG_ERROR("unexpected message type %d", reply_type);
...
int ssh_channel_read(ssh_channel channel, void *dest, uint32_t count) {
    ...
    while (count > 0) {
        if (ssh_buffer_get_len(buf) > 0) {
            ...
            memcpy(dest, ssh_buffer_get(buf), effectivelen);

            nread += effectivelen;
            count -= effectivelen;

            rc = ssh_buffer_pass_bytes(buf, effectivelen);

            if (rc != effectivelen) {
                LOG_ERROR("ssh_buffer_pass_bytes failed");
                goto error;
            }
            ...
            switch (type) {
                case SSH_MSG_CHANNEL_WINDOW_ADJUST:
                    ...
                    channel->remote_window += bytes_to_add;
                    grow_window(channel, channel->remote_window);
                ...
                case SSH_MSG_CHANNEL_DATA:
                    ...
                    effectivelen = MIN(channel->local_maxpacket, channel->local_window);
                    effectivelen = MIN(ssh_string_len(channel_data), effectivelen);

                    ssh_buffer_add_data(buf, ssh_string_data(channel_data), effectivelen);
                ...
                default:
                    ...
                    LOG_ERROR("unknown message type %d", type);
                    goto error;
